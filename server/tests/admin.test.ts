...
describe("settings", () => {
  it("reads defaults and updates settings as admin", async () => {
    ...
    expect(initial.json()).toEqual({ globalPrompt: "", announcement: "", announcementVersion: 0, promptOptimizer: { baseUrl: "", apiKey: "", model: "" }, registration: { enabled: false, dailyQuota: 30 } });
    ...
    expect(updated.json()).toEqual({ globalPrompt: "house style", announcement: "Maintenance tonight", announcementVersion: 1, promptOptimizer: { baseUrl: "", apiKey: "", model: "" }, registration: { enabled: false, dailyQuota: 30 } });
